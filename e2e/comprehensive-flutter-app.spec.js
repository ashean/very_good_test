...
test.describe('Flutter App Comprehensive Tests', () => {
  ...
  test('Test app navigation flow', async ({ page }) => {
    ...
    // 1. Test counter functionality first
    const incrementButton = page.locator('#flt-semantic-node-10');
    ...
    // 3. Go back to main page - reload to ensure we're back to main state
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    ...
    const viewProfilesButton = page.locator('#flt-semantic-node-8');
    await expect(viewProfilesButton).toBeVisible();
    await viewProfilesButton.click();
    await page.waitForTimeout(3000);
    ...
    // 5. Go back and navigate to Database Test
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(5000);
    
    const databaseTestButton = page.locator('#flt-semantic-node-9');
    await expect(databaseTestButton).toBeVisible();
    await databaseTestButton.click();
    await page.waitForTimeout(3000);
  ...
  test('App performance and load testing', async ({ page }) => {
    ...
    // Quick navigation between pages - ensure buttons are visible before clicking
    const addProfileBtn = page.locator('#flt-semantic-node-7');
    await expect(addProfileBtn).toBeVisible();
    await addProfileBtn.click();
    await page.waitForTimeout(1000);
    
    // Go back to main page to ensure buttons are available
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    
    const viewProfilesBtn = page.locator('#flt-semantic-node-8');
    await expect(viewProfilesBtn).toBeVisible();
    await viewProfilesBtn.click();
    await page.waitForTimeout(1000);
    
    // Go back to main page for final navigation
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    
    const databaseTestBtn = page.locator('#flt-semantic-node-9');
    await expect(databaseTestBtn).toBeVisible();
    await databaseTestBtn.click();
    await page.waitForTimeout(1000);
    
    const navigationTime = Date.now() - navigationStart;
    console.log(`Navigation test time: ${navigationTime}ms`);
    
    // Test counter performance - ensure we're back on main page
    await page.goto(APP_URL);
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
